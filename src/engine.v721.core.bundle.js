...
const V7_CORE_VERSION = '7.2.0-map-core-candidate';
const ENGINE_CORE_VERSION = '0.5.2-map-phase2-hotfix';
...
function fxEvade(ctx) {
  ...
  const selected=Array.isArray(ctx.opts?.movePath)?ctx.opts.movePath:null;
  if(selected&&selected.length){
    const moved=MovementResolver.resolve(ctx.state,ctx.attacker,{mode:MOVEMENT_MODE.VOLUNTARY,path:selected,budget:1,retreatOnly:true,source:'EVADE_SELECTED'});
    ctx.log.note += moved.moved?'闪避后退（自选）;':'闪避未位移;';
  }else{
    const moved=ctx.engine.moveAway(ctx, 1, { optional: true });
    ctx.log.note += moved?.moved?'闪避后退（自动）;':'闪避未位移;';
  }
}
...
/** scry：观星。法尤姆【太阳算式】查看并可重排牌堆顶3张。 */
function fxScry(ctx) {
  ctx.engine.scry(ctx.state, ctx.attacker, 3);
}
...
function mapCellAt(state,pos){
  if(!pos)return null;
  const label=state.mapState?.coordIndex?.[`${pos.r},${pos.c}`];
  return label ? state.mapState.cells?.[label] || null : null;
}
function boardCell(state,pos){ return mapCellAt(state,pos); }
function hexStep(state,pos,dir){
  // V7.2 Phase2: boardCell() now resolves authoritative MapState cells; MapState cells do not carry legacy `exists`.
  const c=boardCell(state,pos); if(!c)return null;
...
/**
 * Occupancy is a map rule, not a two-player special case.  Player positions are
 * read directly (so a stale compatibility mirror cannot authorize overlap),
 * then any additional MapState occupants/entities are folded in for forward
 * compatibility with summons, map units and temporary blockers.
 */
function occupiedCellLabels(state,{excludeSide=null}={}){
  const out=new Set();
  for(let i=0;i<(state.players||[]).length;i++){
    if(i===excludeSide)continue;
    const label=mapCellLabel(state,state.players[i]?.pos);if(label)out.add(label);
  }
  for(const [key,value] of Object.entries(state.mapState?.occupants||{})){
    if(String(key)===String(excludeSide))continue;
    const label=typeof value==='string'?value:value?.cell||value?.label||null;if(label)out.add(label);
  }
  for(const ent of Object.values(state.mapState?.entities||{})){
    if(!ent||ent.blocksMovement===false)continue;
    const label=ent.cell||ent.label||null;if(label)out.add(label);
  }
  return out;
}
function canEnterCell(state,side,pos,{ignoreOccupancy=false}={}){
  if(!pos||!isWalkable(state,pos.r,pos.c))return false;
  if(ignoreOccupancy)return true;
  const label=mapCellLabel(state,pos);return !!label&&!occupiedCellLabels(state,{excludeSide:side}).has(label);
}
const MOVEMENT_MODE=Object.freeze({
  VOLUNTARY:'VOLUNTARY',APPROACH:'APPROACH',RETREAT:'RETREAT',FORCED:'FORCED',TELEPORT:'TELEPORT',SHRINK_RELOCATE:'SHRINK_RELOCATE'
});
...
function isWalkable(state,r,c){
  const cell=mapCellAt(state,{r,c});
  return !!cell && cell.walkable!==false && cell.zone!==false;
}
...
function cellTerrain(state,r,c){ const cell=mapCellAt(state,{r,c});return cell?(cell.terrain||null):null; }

function edgeForMove(state,from,to){
  const cell=mapCellAt(state,from);
  return Object.values(cell?.neighborsByDir||{}).find(e=>e.row===to.r&&e.col===to.c)||null;
}
function getReachableMovePaths(state,side,budget,{approachOnly=false,retreatOnly=false,start=null}={}){
  ...
  while(queue.length){
    const cur=queue.shift(),cell=mapCellAt(state,cur.pos);
    for(const e of Object.values(cell?.neighborsByDir||{})){
      ...
      if(!canTraverse(state,cur.pos,next)||!canEnterCell(state,side,next))continue;
      ...
      if(retreatOnly&&dist(next,opp.pos)<startD)continue;
      ...
      if(!retreatOnly||dist(next,opp.pos)>startD)out.push({dest:{...next},path,spent,remaining:budget-spent,relation:dist(next,opp.pos)>startD?'后退':dist(next,opp.pos)<startD?'前进':'侧移'});
      if(!stop&&spent<budget)queue.push({pos:next,path,spent});
    }
  }return out;
}
function executeMovementPath(state,side,path,budget,{approachOnly=false,retreatOnly=false}={}){
  ...
  let spent=0,terminatedByTerrain=false;
  for(const raw of path){const next={r:Number(raw.r),c:Number(raw.c)},e=edgeForMove(state,p.pos,next);
    ...
    if(!canEnterCell(state,side,next))throw new Error('目标格已被占据');
    ...
    if(retreatOnly&&dist(next,opp.pos)<dist(origin,opp.pos))throw new Error('该移动不能接近对手');
    p.pos=next;
    if((e.tags||[]).includes('end_move')||['mud','shallow_water'].includes(cellTerrain(state,next.r,next.c))){terminatedByTerrain=true;break;}
  }
  if(retreatOnly&&dist(p.pos,opp.pos)<=dist(origin,opp.pos))throw new Error('闪避移动必须使你远离对手');
  return {moved:origin.r!==p.pos.r||origin.c!==p.pos.c,spent,origin,dest:{...p.pos},terminatedByTerrain};
}
function pickAutoMovementPath(state,side,budget,mode){
  const p=state.players[side],opp=state.players[1-side],startD=dist(p.pos,opp.pos);
  if(mode===MOVEMENT_MODE.APPROACH&&startD<=CONST.MELEE_RANGE)return null;
  const paths=getReachableMovePaths(state,side,budget,{approachOnly:mode===MOVEMENT_MODE.APPROACH});
  const filtered=paths.filter(x=>mode===MOVEMENT_MODE.APPROACH?dist(x.dest,opp.pos)<startD:dist(x.dest,opp.pos)>startD);
  filtered.sort((a,b)=>{
    const da=dist(a.dest,opp.pos),db=dist(b.dest,opp.pos);
    if(da!==db)return mode===MOVEMENT_MODE.APPROACH?da-db:db-da;
    if(a.spent!==b.spent)return a.spent-b.spent;
    return `${a.dest.r},${a.dest.c}`.localeCompare(`${b.dest.r},${b.dest.c}`);
  });
  return filtered[0]||null;
}
const MovementResolver=Object.freeze({
  resolve(state,side,{mode=MOVEMENT_MODE.VOLUNTARY,budget=0,path=null,direction=null,steps=null,destination=null,approachOnly=false,retreatOnly=false,source='MOVE',emit=true}={}){
    const p=state.players[side];if(!p)throw new Error('非法移动方');
    const origin={...p.pos};let result={moved:false,spent:0,origin,dest:{...origin},blocked:false,terminatedByTerrain:false,mode,source};
    if(mode===MOVEMENT_MODE.TELEPORT||mode===MOVEMENT_MODE.SHRINK_RELOCATE){
      if(!destination)throw new Error('缺少迁移目标');
      const next={r:Number(destination.r),c:Number(destination.c)};
      if(!canEnterCell(state,side,next))throw new Error('迁移目标不可进入');
      p.pos=next;result={...result,moved:origin.r!==next.r||origin.c!==next.c,dest:{...next}};
    }else if(mode===MOVEMENT_MODE.FORCED){
      const maxSteps=Math.max(0,Number(steps??budget)||0);let movedSteps=0,terrainCost=0;
      if(!direction)throw new Error('强制位移缺少方向');
      for(let i=0;i<maxSteps;i++){
        const next=hexStep(state,p.pos,direction);
        if(!next||!canTraverse(state,p.pos,next)||!canEnterCell(state,side,next)){result.blocked=true;break;}
        const e=edgeForMove(state,p.pos,next);terrainCost+=Math.max(1,Number(e?.cost)||1);p.pos={r:next.r,c:next.c};movedSteps++;
        if((e?.tags||[]).includes('end_move')||['mud','shallow_water'].includes(cellTerrain(state,p.pos.r,p.pos.c))){result.terminatedByTerrain=true;break;}
      }
      result={...result,moved:movedSteps>0,spent:movedSteps,terrainCost,steps:movedSteps,dest:{...p.pos}};
    }else{
      const moveBudget=Math.max(0,Number(budget)||0);let selected=path;
      if(mode===MOVEMENT_MODE.APPROACH||mode===MOVEMENT_MODE.RETREAT){
        const best=pickAutoMovementPath(state,side,moveBudget,mode);selected=best?.path||null;
        if(!selected)return result;
      }else if((!selected||!selected.length)&&direction){
        const next=hexStep(state,p.pos,direction);selected=next?[{r:next.r,c:next.c}]:null;
      }
      if(!selected||!selected.length)return result;
      const moved=executeMovementPath(state,side,selected,moveBudget,{approachOnly:approachOnly||mode===MOVEMENT_MODE.APPROACH,retreatOnly});
      result={...result,...moved,mode,source};
    }
    if(emit&&result.moved){
      if(mode===MOVEMENT_MODE.FORCED||mode===MOVEMENT_MODE.SHRINK_RELOCATE||mode===MOVEMENT_MODE.TELEPORT)
        emitDistanceDomainEvents(state,side,result.origin,result.dest,{forced:true,source});
      else finalizeMoveMechanics(state,side,result.origin,{source});
    }
    return result;
  }
});
function applySelectedMovePath(state,side,path,budget,{approachOnly=false,source='MOVE',emit=false}={}){
  return MovementResolver.resolve(state,side,{mode:MOVEMENT_MODE.VOLUNTARY,path,budget,approachOnly,source,emit});
}
...
function getPendingChoiceOptions(state) {
  ...
  return {
    id:tx.id,type:tx.type,side:tx.side,purpose:tx.purpose||null,count:tx.count||0,
    // 对 UI 的顺序契约统一为“牌堆顶 -> 更深处”，提交时同样使用该顺序。
    cards:tx.cards.slice().reverse().map(c=>clone(c))
  };
}
function submitPendingChoice(state, choiceId, order) {
  ...
  const stableKey=(c)=>c.instanceId||c.id||c.name;
  const expected=tx.cards.map(stableKey).sort();
  const actual=order.map(stableKey).sort();
  if(expected.join('|')!==actual.join('|')||new Set(actual).size!==actual.length)throw new Error('提交内容必须与原牌集合完全一致');
  const p=state.players[tx.side];
  // order 使用“牌堆顶 -> 更深处”；deck 尾部为牌堆顶，因此落盘时反转。
...
function knockback(ctx,n){
  ...
  const dir=hexAwayDirection(state,atk.pos,def.pos);
  const push=dir?MovementResolver.resolve(state,defender,{mode:MOVEMENT_MODE.FORCED,direction:dir,steps:amount,source:'KNOCKBACK'}):{moved:false,steps:0,blocked:true};
  const moved=push.steps||0,wallHit=!!push.blocked;
  ...
  if(moved>0&&atk.hero.id==='chiyu'&&!atk.mechanics.knockAdvanceUsed){
    atk.mechanics.knockAdvanceUsed=true;
    MovementResolver.resolve(state,attacker,{mode:MOVEMENT_MODE.APPROACH,budget:1,source:'KNOCK_ADVANCE'});
  }
...
function finalizeMoveMechanics(state,side,before,{source='MOVE'}={}){
  const p=state.players[side];
  if(before.r===p.pos.r&&before.c===p.pos.c)return false;
  emitDistanceDomainEvents(state,side,before,p.pos,{forced:false,source});
...
function effectiveAutoMoveBudget(state,side,n){
  const p=state.players[side];let budget=n+(p.mechanics.baiyeWaterMoveArmed?1:0);
  const frozen=getStatus(p,PERSISTENT.FROZEN);if(frozen){budget=Math.max(0,budget-1);removeStatus(state,side,PERSISTENT.FROZEN);}
  return budget;
}
function moveToward(ctx,n,{optional=false}={}){
  const {state,attacker}=ctx,budget=effectiveAutoMoveBudget(state,attacker,n);
  return MovementResolver.resolve(state,attacker,{mode:MOVEMENT_MODE.APPROACH,budget,source:'MOVE_TOWARD'});
}
function moveTowardWithinBudget(ctx,n){
  const {state,attacker}=ctx;
  return MovementResolver.resolve(state,attacker,{mode:MOVEMENT_MODE.APPROACH,budget:n,source:'MOVE_TOWARD_BUDGET'}).moved;
}
...
function moveSelectedOrToward(ctx,n){
  const {state,attacker,opts,card}=ctx,selected=Array.isArray(opts?.movePath)?opts.movePath:null,budget=effectiveAutoMoveBudget(state,attacker,n);
  const r=selected&&selected.length
    ?MovementResolver.resolve(state,attacker,{mode:MOVEMENT_MODE.VOLUNTARY,path:selected,budget,approachOnly:!!card?.approachOnly,source:'SELECTED_MOVE'})
    :MovementResolver.resolve(state,attacker,{mode:MOVEMENT_MODE.APPROACH,budget,source:'AUTO_APPROACH'});
  if(selected&&selected.length&&!r.moved)throw new Error('没有完成移动');
  return r;
}
function moveAway(ctx,n,{optional=false}={}){
  const {state,attacker}=ctx;
  return MovementResolver.resolve(state,attacker,{mode:MOVEMENT_MODE.RETREAT,budget:n,source:'MOVE_AWAY'});
}
...
function continueResolution(state, attackerSide, card, log, opts = {}) {
  ...
  if (card.type === 'attack') {
    ...
    if(atk.hero.id==='youying'&&atk.mechanics.wanderMovesUsed<2&&Array.isArray(opts.wanderPath)&&opts.wanderPath.length){
      try{
        const moved=MovementResolver.resolve(state,attackerSide,{mode:MOVEMENT_MODE.VOLUNTARY,path:opts.wanderPath,budget:atk.mechanics.flowBreak?2:1,source:'WANDER'});
        if(moved.moved){
          atk.mechanics.wanderMovesUsed+=1;log.note+=`游步${moved.spent};`;
...
function expansionEndPipeline(state, context = {}) {
  ...
  if (voluntary && active.hero.id === 'qiu013' && v7LedgerValue(state,activeSide,'attacksResolved') === 0
    ...
    // V7.2.4：整备型被动摸牌只补到软目标5张；主动卡牌/技能仍可超量。
    const suppressDrawBudget = Math.max(0, CONST.HAND_LIMIT - active.hand.length);
    const drawn = suppressDrawBudget > 0 ? drawCards(state, activeSide, Math.min(1, suppressDrawBudget)).length : 0;
    state.log.push({ type: 'mechanic', side: activeSide, mechanic: 'suppress_break', restored, drawn, drawBlocked: drawn === 0 && active.hand.length >= CONST.HAND_LIMIT, reason: endReason });
  }
  ...
  const rp = mapObjectivePosition(state,'resource-main');
  if (rp && active.pos.r === rp.r && active.pos.c === rp.c) {
    ...
    // V7.2.4：地图资源点摸牌也只补到软目标5张，气/能奖励不变。
    const resourceDrawBudget = Math.max(0, CONST.HAND_LIMIT - active.hand.length);
    const resourceDrawn = tier.draw > 0 && resourceDrawBudget > 0
      ? drawCards(state, activeSide, Math.min(tier.draw, resourceDrawBudget)).length
      : 0;
    state.log.push({
      type: 'resource', side: activeSide, streak: active.mechanics.resourceStreak,
      qi: tier.qi, energy: tier.energy, draw: resourceDrawn,
      drawBlocked: tier.draw > resourceDrawn && active.hand.length >= CONST.HAND_LIMIT,
    });
  } else {
  ...
  const sp = activeSupplyPosition(state);
  if (sp && active.pos.r === sp.r && active.pos.c === sp.c) {
...
function _finishExpansion(state, activeSide) {
  ...
  // ---- 危险区 + 持续伤害 ----
  // 缩域封锁区：站在封锁格受1伤
  const cell=mapCellAt(state,active.pos);
  ...
  // V7.2.9 anti-brick：判断“主动回合是否至少有一张能作为行动入口的牌”。
  // 反击牌只在 RESPONSE_WINDOW 可用、追击牌只在 CHASE_WINDOW 可用，不能阻止死手换手；
  // 同时 move/buff/resource/control/heal/state 等主动牌应视为可行动，避免被旧逻辑误换。
  const hasPreAttackCard = active.hand.some((c) =>
    (c.type === 'attack' && (c.timing === 'starter' || c.modes?.active || c.allowActive))
    || ['move', 'buff', 'resource', 'control', 'heal', 'state'].includes(c.type)
  );
  if (!hasPreAttackCard && !active.mechanics.handRedrawUsedThisRound) {
    // 整备换手：主动回合整手均为追击/反击等非主动牌时，弃全部手牌并摸等量。
  ...
  if (active.hp <= 0) {
    ...
    newActive.mechanics.lafengSeize=false;
    newActive.mechanics.lafengSeizeFollow=true;
    // Any CHASE_WINDOW is expansion-scoped. The previous expansion has already been closed above,
    // so create the new authoritative expansion before opening the chase window.
    ensureExpansion(state,nextMainActionSide);
    state.phase=PHASE.CHASE_WINDOW;
...
// ---------------------------------------------------------------------------
// V7.2 Map Core：Engine 内权威地图状态、目标与位移领域事件
// ---------------------------------------------------------------------------
function mapCellLabel(state,pos){
  const indexed=state.mapState?.coordIndex?.[`${pos?.r},${pos?.c}`];
  if(indexed)return indexed;
  const c=state.board?.cells?.[pos?.r]?.[pos?.c];
  return c?.label || `${pos?.r},${pos?.c}`;
}
function buildAuthoritativeMapState(state){
  const cells={},coordIndex={};
  for(const row of state.board?.cells||[])for(const c of row||[]){
    if(!c||c.exists===false)continue;
    const label=c.label||`${c.row??0},${c.col??0}`;
    const neighborsByDir={};
    for(const [dir,e] of Object.entries(c.neighborsByDir||{})){
      neighborsByDir[dir]={to:e.to||null,row:e.row??null,col:e.col??null,passable:e.passable!==false,
        cost:e.cost??1,tags:[...(e.tags||[])],ramp:!!e.ramp,cliff:!!e.cliff};
    }
    cells[label]={label,row:c.row??null,col:c.col??null,q:c.q??null,r:c.axialR??null,
      terrain:c.terrain||null,height:c.height||0,walkable:c.walkable!==false&&!c.obstacle,
      zone:c.zone!==false,danger:!!c.danger,blocksLOS:!!c.blocksLOS,neighborsByDir};
    coordIndex[`${c.row??0},${c.col??0}`]=label;
  }
  const resource=state.board?.resource;
  const supplies=state.board?.supplyRotation||[];
  return {
    version:'7.2-mapstate-2',
    mapId:state.board?.mapId||'legacy-board',
    mapName:state.board?.mapName||'Legacy Board',
    grid:state.board?.hex?'hex_axial_pointy':'legacy_grid',
    section:state.section,
    coordIndex,
    cells,
    shrinkStages:clone(state.board?.shrinkStages||[]),
    objectives:{
      resource:resource?{id:'resource-main',type:'RESOURCE',cell:resource.label||mapCellLabel(state,resource),controller:null}:null,
      supplies:supplies.map((p,i)=>({id:`supply-${i}`,type:'SUPPLY',cell:p.label||mapCellLabel(state,p),active:i===(state.supplyIndex%(supplies.length||1))}))
    },
    occupants:{
      0:mapCellLabel(state,state.players[0].pos),
      1:mapCellLabel(state,state.players[1].pos)
    }
  };
}
function mapPositionForLabel(state,label){
  const c=state.mapState?.cells?.[label];
  return c?{r:c.row,c:c.col,label:c.label}:null;
}
function mapObjectivePosition(state,id){
  const o=state.mapState?.objectives?.resource?.id===id?state.mapState.objectives.resource:null;
  return o?mapPositionForLabel(state,o.cell):null;
}
function activeSupplyPosition(state){
  const supplies=state.mapState?.objectives?.supplies||[];
  if(!supplies.length)return null;
  const active=supplies.find(x=>x.active)||supplies[state.supplyIndex%supplies.length];
  return active?mapPositionForLabel(state,active.cell):null;
}
function mirrorMapStateToLegacyBoard(state){
  // Compatibility mirror only: legacy UI/debug readers may still inspect board,
  // but all Map rules above read mapState.
  for(const c of Object.values(state.mapState?.cells||{})){
    const legacy=state.board?.cells?.[c.row]?.[c.col];
    if(!legacy)continue;
    legacy.zone=c.zone!==false; legacy.danger=!!c.danger;
  }
}
function syncMapState(state){
  if(!state.mapState) state.mapState=buildAuthoritativeMapState(state);
  state.mapState.section=state.section;
  state.mapState.occupants={
    0:mapCellLabel(state,state.players[0].pos),
    1:mapCellLabel(state,state.players[1].pos)
  };
  const supplies=state.mapState.objectives?.supplies||[];
  for(let i=0;i<supplies.length;i++)supplies[i].active=i===(state.supplyIndex%(supplies.length||1));
  const rp=state.mapState.objectives?.resource;
  if(rp){
    const p0=mapPositionForLabel(state,rp.cell),p1=p0;
    const on0=!!p0&&state.players[0].pos.r===p0.r&&state.players[0].pos.c===p0.c;
    const on1=!!p1&&state.players[1].pos.r===p1.r&&state.players[1].pos.c===p1.c;
    rp.controller=on0&&!on1?0:on1&&!on0?1:null;
  }
  mirrorMapStateToLegacyBoard(state);
  return state.mapState;
}
function emitDistanceDomainEvents(state,side,before,after,{forced=false,source='MOVE'}={}){
  if(!before||!after||(before.r===after.r&&before.c===after.c))return;
  if(!state.expansion) ensureExpansion(state,side);
  const opp=state.players[1-side];
  const beforeD=dist(before,opp.pos),afterD=dist(after,opp.pos),delta=afterD-beforeD;
  emitV7Event(state,{type:'POSITION_CHANGED',actorId:side,side,from:{...before},to:{...after},
    distance:dist(before,after),forced,source,approachDelta:Math.max(0,-delta),retreatDelta:Math.max(0,delta),lateral:delta===0});
  emitV7Event(state,{type:'DISTANCE_CHANGED',actorId:side,side,before:beforeD,after:afterD,delta,forced,source});
  emitV7Event(state,{type:delta<0?'APPROACHED':delta>0?'MOVED_AWAY':'LATERAL_MOVE',
    actorId:side,side,before:beforeD,after:afterD,distance:Math.abs(delta),forced,source});
  syncMapState(state);
}
function nearestLegalRelocation(state,side){
  const p=state.players[side],opp=state.players[1-side],cands=[];
  for(const c of Object.values(state.mapState?.cells||{})){
    if(!c||c.walkable===false||c.zone===false)continue;
    if(!canEnterCell(state,side,{r:c.row,c:c.col}))continue;
    cands.push({r:c.row,c:c.col,label:c.label});
  }
  cands.sort((a,b)=>dist(p.pos,a)-dist(p.pos,b)||String(a.label).localeCompare(String(b.label)));
  return cands[0]||null;
}
function applyMapShrinkStage(state,section){
  const cells=Object.values(state.mapState?.cells||{});
  const stages=state.mapState?.shrinkStages||[];
  if(stages.length){
    const stage=stages.find(s=>s.section===section)||stages.at(-1);
    const danger=new Set(stage?.danger||[]),locked=new Set(stage?.locked||[]);
    for(const c of cells){c.zone=!locked.has(c.label);c.danger=danger.has(c.label);}
  }else{
    // Legacy map fallback is computed into MapState, never into board.
    const rows=cells.map(c=>c.row),cols=cells.map(c=>c.col);
    const maxR=Math.max(...rows),maxC=Math.max(...cols),level=section-1;
    for(const c of cells){
      const ring=Math.min(c.row,c.col,maxR-c.row,maxC-c.col);
      if(level>0&&ring<level)c.zone=false;
      c.danger=false;
    }
  }
}
/** 缩域：MapState 为唯一规则事实源；board 仅做兼容镜像。 */
function shrinkZone(state){
  applyMapShrinkStage(state,state.section);
  // Phase4 shrink contract:
  // 1) a living actor can be force-relocated at most once for the same section;
  // 2) defeated actors are never resurrected/moved by shrink cleanup;
  // 3) relocation always goes through the single MovementResolver SHRINK_RELOCATE path.
  state._shrinkRelocatedSectionBySide=state._shrinkRelocatedSectionBySide||{};
  for(const side of [0,1]){
    const p=state.players[side],cell=mapCellAt(state,p.pos);
    if(!p||p.hp<=0)continue;
    if(cell&&cell.zone!==false)continue;
    if(state._shrinkRelocatedSectionBySide[side]===state.section)continue;
    const before={...p.pos},dest=nearestLegalRelocation(state,side);
    if(dest){
      const moved=MovementResolver.resolve(state,side,{mode:MOVEMENT_MODE.SHRINK_RELOCATE,destination:dest,source:'SHRINK_RELOCATION'});
      if(moved.moved){
        state._shrinkRelocatedSectionBySide[side]=state.section;
        state.log.push({type:'map_relocation',side,reason:'shrink',section:state.section,from:before,to:{...p.pos}});
      }
    }
  }
  syncMapState(state);
  emitV7Event(state,{type:'MAP_SHRINK_APPLIED',actorId:null,payload:{section:state.section}});
}
...
const HEROES = {
  ...
  qiu013: {
    ...
    mechanics: [
      ...
      { id: 'suppress_break', text: '抑制崩坏：每大回合1次，主动收势且未攻击时，回2生命；若手牌少于5，再摸1。' }, // V6 R4: 不限次导致憋尿拖超时成最优解
...
function newGame(heroAId, heroBId, options = {}) {
  ...
  state.mapState=buildAuthoritativeMapState(state);
  syncMapState(state);
...
function tacticalStep(state,side,dir,opts={}){
  return transact(state,draft=>{if(draft.phase!==PHASE.PRE_ATTACK)throw new Error('当前不能战术步');if(ruleInitiativeSide(draft)!==side)throw new Error('无进攻权');
    ...
    let moved;
    if(Array.isArray(opts.movePath)&&opts.movePath.length)moved=MovementResolver.resolve(draft,side,{mode:MOVEMENT_MODE.VOLUNTARY,path:opts.movePath,budget,source:'TACTICAL_STEP'});
    else{if(!HEX_DIRS[dir])throw new Error(`非法六边方向：${dir}`);moved=MovementResolver.resolve(draft,side,{mode:MOVEMENT_MODE.VOLUNTARY,direction:dir,budget,source:'TACTICAL_STEP'});}
    if(!moved.moved)throw new Error('目标方向不可行走');p.mechanics.tacticalStepUsed=true;
...
function applySkill(state,side,skillId,opts={}) {
  ...
  switch (skillId) {
    ...
    case 'swift_double':{if(Array.isArray(opts.movePath)&&opts.movePath.length)MovementResolver.resolve(state,side,{mode:MOVEMENT_MODE.VOLUNTARY,path:opts.movePath,budget:2,source:'SWIFT_DOUBLE'});else moveToward({state,attacker:side,defender:1-side},2);p.mechanics.swiftDouble=true;break}
    case 'light_breath':{if(Array.isArray(opts.movePath)&&opts.movePath.length)MovementResolver.resolve(state,side,{mode:MOVEMENT_MODE.VOLUNTARY,path:opts.movePath,budget:1,source:'LIGHT_BREATH'});else moveToward({state,attacker:side,defender:1-side},1);const def=state.players[1-side];p.energy=Math.min(CONST.ENERGY_MAX,p.energy+1);def.energy=Math.max(CONST.ENERGY_DRAIN_FLOOR,def.energy-1);break}
    // 玄医
    case 'herb_revive': {
      p.statusSlots.control = [];
      p.statusSlots.persistent = p.statusSlots.persistent.filter(
...
function useUltimate(state, side, ultId) {
  return transact(state, (draft) => {
    ...
    if (ultId === 'ten_sec_kill') {
      ...
      // 突进2 + 5伤 + 击退2
      MovementResolver.resolve(draft,side,{mode:MOVEMENT_MODE.APPROACH,budget:ult.dash||0,source:'ULT_DASH'});
      damage(draft, 1 - side, dmg, ult.name);
...
// ============================================================================
// V7.2.10 高手 AI Phase 1：有限预算对手感知前瞻
// 仅当前游戏 AI 单次决策同步执行；无后台轮询/定时器/Worker/GPU/全局事件钩子/跨决策缓存。
// ============================================================================
const V7210_LOOKAHEAD=Object.freeze({rootCandidates:4,branchCandidates:3,pliesAfterRoot:2,maxNodes:36,maxMillis:6,minHardwareConcurrency:3});
function v7210Now(){return(typeof performance!=='undefined'&&performance&&typeof performance.now==='function')?performance.now():Date.now()}
function v7210CanSearch(){
  if(typeof document!=='undefined'&&document&&document.hidden)return false;
  if(typeof navigator!=='undefined'&&navigator){const hc=Number(navigator.hardwareConcurrency||0);if(hc>0&&hc<V7210_LOOKAHEAD.minHardwareConcurrency)return false}
  return true
}
function v7210StatusStacks(p,id){if(!p?.statusSlots)return 0;let n=0;for(const k of['persistent','control','instant'])for(const s of p.statusSlots[k]||[])if(s?.id===id)n+=(s.stacks||1);return n}
function v7210PositionValue(state,side){
  const p=state.players?.[side];if(!p?.pos)return 0;const cell=boardCell(state,p.pos.r,p.pos.c);let v=cell?.zone===false?-8:2;
  if(cell?.terrain==='highland')v+=1.5;if(cell?.terrain==='bush')v+=0.5;
  const rp=mapObjectivePosition(state,'resource-main');if(rp)v+=Math.max(0,2.5-dist(p.pos,rp)*0.45);return v
}
function v7210OwnHandQuality(state,side){
  const p=state.players?.[side],hand=Array.isArray(p?.hand)?p.hand:[];if(!hand.length)return-1;
  let v=Math.min(6,hand.length)*.35,cheap=Infinity,counters=0,follows=0;
  for(const c of hand){if(c.type==='attack'){cheap=Math.min(cheap,c.cost||0);if(c.timing==='follow')follows++}if(c.type==='counter')counters++}
  if(cheap<Infinity&&(p.energy||0)>=cheap)v+=1.25;v+=Math.min(2,follows)*.35;
  const opp=state.players?.[1-side];if(counters&&(opp?.energy||0)>=1)v+=Math.min(2,counters)*.9;return v
}
function v7210Evaluate(state,rootSide){
  const me=state.players?.[rootSide],opp=state.players?.[1-rootSide];if(!me||!opp)return 0;
  if(state.winner!=null){if(state.winner==='draw')return 0;return state.winner===rootSide?100000:-100000}
  const mm=me.hero?.hp||me.maxHp||Math.max(1,me.hp||1),om=opp.hero?.hp||opp.maxHp||Math.max(1,opp.hp||1);
  let v=((me.hp||0)/mm-(opp.hp||0)/om)*42+((me.energy||0)-(opp.energy||0))*2.1+((me.qi||0)-(opp.qi||0))*1.6;
  v+=(v7210StatusStacks(me,'shield')-v7210StatusStacks(opp,'shield'))*2.2;
  if(getPosture(me)!==POSTURE.NORMAL)v-=2.5;if(getPosture(opp)!==POSTURE.NORMAL)v+=2.5;
  for(const id of[CONTROL.STIFF,CONTROL.SEALED,CONTROL.ROOTED,PERSISTENT.BURN,PERSISTENT.POISON]){
    if(id&&hasStatus(me,id))v-=1.2;if(id&&hasStatus(opp,id))v+=1.2
  }
  v+=v7210PositionValue(state,rootSide)-v7210PositionValue(state,1-rootSide)+v7210OwnHandQuality(state,rootSide);
  const actor=ruleActorSide(state);v+=actor===rootSide?2.2:-2.2;
  if(state.phase===PHASE.RESPONSE_WINDOW&&state.pendingCard)v+=state.pendingCard.attackerSide===rootSide?1.8:-1.8;
  if(me.pos&&opp.pos&&dist(me.pos,opp.pos)<=CONST.MELEE_RANGE){v+=((me.energy||0)-(opp.energy||0))*.55;v+=actor===rootSide?1.2:-1.2}
  return v
}
function v7210OrderScore(state,a,side){
  const me=state.players?.[side],opp=state.players?.[1-side];let s=0,d=immediateDamage(a);if(d>0)s+=d*12;if(opp&&d>=(opp.hp||Infinity))s+=1000;s-=costOf(a)*1.5;
  if(a.kind==='counter')s+=16+d*4;if(a.kind==='use_ultimate')s+=24+d*6;if(a.kind==='struggle'||a.kind==='burst_struggle')s+=4;
  if(a.kind==='pass')s-=1;if(a.kind==='end')s-=2;if(isResourceAction(a))s+=3;
  if(isHealAction(a)){const mx=me?.hero?.hp||me?.maxHp||me?.hp||1;s+=Math.min(Math.max(0,mx-(me?.hp||0)),a.heal||2)*5}
  if(isMoveAction(a)&&me?.pos&&opp?.pos&&a.dest){s+=(dist(me.pos,opp.pos)-dist(a.dest,opp.pos))*4;const cell=boardCell(state,a.dest.r,a.dest.c);if(cell?.zone===false)s-=18;if(cell?.terrain==='highland')s+=2}
  return s
}
function v7210Top(state,actions,side,n){return actions.map((a,i)=>({a,i,s:v7210OrderScore(state,a,side)})).sort((x,y)=>(y.s-x.s)||(x.i-y.i)).slice(0,n).map(x=>x.a)}
function v7210UnknownCard(zone,index){
  const id=`__v7210_unknown_${zone}_${index}`;
  return {instanceId:id,id,name:'__hidden_unknown__',type:'hidden_unknown',timing:'hidden',cost:999,damage:0,range:0,effect:''}
}
// Phase1 hidden-information boundary: search is built from the root player's information set.
// Opponent hand identities and both deck orders are not available to lookahead.
// Counts are preserved with inert placeholders so public hand/deck size remains stable without creating hidden-card actions.
function v7210PerspectiveState(state,rootSide){
  const s=clone(state);
  for(let side=0;side<2;side++){
    const p=s.players?.[side];if(!p)continue;
    if(side!==rootSide&&Array.isArray(p.hand))p.hand=p.hand.map((_,i)=>v7210UnknownCard(`hand_${side}`,i));
    if(Array.isArray(p.deck))p.deck=p.deck.map((_,i)=>v7210UnknownCard(`deck_${side}`,i));
  }
  return s
}
function v7210Search(state,rootSide,depth,budget){
  if(state.winner!=null||depth<=0)return v7210Evaluate(state,rootSide);
  if(budget.nodes>=V7210_LOOKAHEAD.maxNodes||v7210Now()>=budget.deadline)return v7210Evaluate(state,rootSide);
  const acts=simGetLegalActions(state);if(!acts.length)return v7210Evaluate(state,rootSide);const actor=ruleActorSide(state),cand=v7210Top(state,acts,actor,V7210_LOOKAHEAD.branchCandidates);
  let best=actor===rootSide?-Infinity:Infinity;
  for(const a of cand){if(budget.nodes>=V7210_LOOKAHEAD.maxNodes||v7210Now()>=budget.deadline)break;budget.nodes++;const branch=clone(state),next=applyAction(branch,a),val=v7210Search(next,rootSide,depth-1,budget);best=actor===rootSide?Math.max(best,val):Math.min(best,val)}
  return Number.isFinite(best)?best:v7210Evaluate(state,rootSide)
}
function v7210OpponentAwareInternalChoice(state,legal,ctx,heuristicBest){
  if(!v7210CanSearch()||!Array.isArray(state?.players)||legal.length<2)return heuristicBest;
  if(![PHASE.PRE_ATTACK,PHASE.CHASE_WINDOW,PHASE.RESPONSE_WINDOW].includes(state.phase))return heuristicBest;
  const rootSide=ctx.playerId==='A'?0:1,roots=[];if(heuristicBest)roots.push(heuristicBest);
  for(const a of v7210Top(state,legal,rootSide,V7210_LOOKAHEAD.rootCandidates+1)){if(roots.length>=V7210_LOOKAHEAD.rootCandidates)break;if(!roots.some(x=>x.actionId===a.actionId))roots.push(a)}
  const searchBase=v7210PerspectiveState(state,rootSide);
  const budget={nodes:0,deadline:v7210Now()+V7210_LOOKAHEAD.maxMillis};let best=heuristicBest,bv=-Infinity;
  for(const a of roots){if(budget.nodes>=V7210_LOOKAHEAD.maxNodes||v7210Now()>=budget.deadline)break;budget.nodes++;const next=applyAction(clone(searchBase),a),future=v7210Search(next,rootSide,V7210_LOOKAHEAD.pliesAfterRoot,budget),value=future+(heuristicBest&&a.actionId===heuristicBest.actionId?0.35:0);if(value>bv){bv=value;best=a}}
...
function simGetLegalActions(state) {
  ...
  if (state.pendingChoice) {
    ...
    if (choice?.type === 'DECK_ORDER') {
      return decorateLegalActions(state, [{
        kind: 'scry_order',
        choiceId: choice.id,
        cardInstanceIds: choice.cards.map(c => c.instanceId || c.id || c.name),
        cardNames: choice.cards.map(c => c.name),
        cost: 0
      }]);
    }
  }
  ...
  if (Array.isArray(la.cards)) {
    la.cards.forEach((card, idx) => {
      ...
      if (phase === 'RESPONSE_WINDOW') {
        const baseCounter={ kind: 'counter', cardInstanceId: card.instanceId || card.id || card.name, cardKey: card.instanceId || card.id || card.name, cardName: card.name, damage: card.damage ?? 0, cost: card.cost ?? 0, effect: card.effect ?? '' };
        if(card.effect==='evade'){
          const paths=getReachableMovePaths(state,actingSide,1,{retreatOnly:true});
          if(paths.length)for(const item of paths)out.push({...baseCounter,movePath:item.path,dest:item.dest,moveSpent:item.spent});
          else out.push(baseCounter);
        }else out.push(baseCounter);
      } else {
  ...
  for (const skill of la.skills ?? []) {
    if (!failed.has(`skill:${skill.id}`)) {
      ...
      if(skill.moveBudget){for(const item of getReachableMovePaths(state,actingSide,skill.moveBudget,{approachOnly:!!skill.approachOnly}))out.push({...base,movePath:item.path,dest:item.dest,moveSpent:item.spent})}
      else out.push(base);
    }
  }
...
class GameEngineCore {
  ...
  dispatch(command){
    ...
    const beforeLedger=(before.expansion?.eventLedger?.events||[]).length;
    const eventDelta=[];
    ...
    // 某些旧事务路径会在 clone 内重置命令事件 sink；V7.2 以 ExpansionLedger 的增量补齐公开事件。
    if(!eventDelta.length){
      const afterEvents=d.state.expansion?.eventLedger?.events||[];
      for(const e of afterEvents.slice(beforeLedger))eventDelta.push(clone(e));
    }
    return {ok:true,events:eventDelta.map(e=>clone(e)),
      eventSequence:eventDelta.length?{from:eventDelta[0].sequence??eventDelta[0].seq??null,to:eventDelta[eventDelta.length-1].sequence??eventDelta[eventDelta.length-1].seq??null}:null,
  ...
  queryMapState() {
    const d=__coreData(this); syncMapState(d.state); return clone(d.state.mapState);
  }
...
function __createAI(strategy, heroId, side='B', seed=1) {
  ...
  return Object.freeze({
    choose(core){
      ...
      try {
        const aiCtx={rng,heroId,playerId:idx===0?'A':'B',opponentId:idx===0?'B':'A',engine:null};
        let action=fn(obs,legal,aiCtx);
        if(action && legal.some(a=>a.actionId===action.actionId) && (strategy==='tactical'||strategy==='role')){
          action=v7210OpponentAwareInternalChoice(d.state,legal,aiCtx,action);
        }
